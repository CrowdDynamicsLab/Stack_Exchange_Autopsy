    ...
    for i in range(T1, length):
        ...
        for j in range(i - T1, i):
            ...
            if observed_q[i][0] - observed_q[j][0] <= T1:
                tmp_sum += C1 * observed_q[j][1] * (math.exp(observed_q[i][0] - observed_q[j][0]))
            if observed_a[i][0] - observed_a[j][0] <= T1:
                tmp_sum += C2 * observed_a[j][1] * (math.exp(observed_a[i][0] - observed_a[j][0]))
            if observed_c[i][0] - observed_c[j][0] <= T1:
                tmp_sum += C3 * observed_c[j][1] * (math.exp(observed_c[i][0] - observed_c[j][0]))
    ...
    for i in range(T2, length):
        ...
        for j in range(i - T2, i):
            if observed_a[i][0] - observed_a[j][0] <= T2:
                tmp_sum += C2 * observed_a[j][1] * (math.exp(observed_a[i][0] - expected_a[j][0]))
...
def main(argv):
    ...
    for channel in all_channels:
        ...
        py.plot(fig, filename = channel_name + '_daily_ver3')
